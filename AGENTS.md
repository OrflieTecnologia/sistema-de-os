...
### Fase 16: Atualização Automática das Páginas (sem F5) (Concluída)
- [x] **Auto-refresh das listagens e relatórios:** Hook `useAutoRefresh` (`src/lib/use-auto-refresh.ts`) que dispara um callback ao focar/voltar para a aba (`focus`/`visibilitychange`) e a cada ~30s, **apenas com a aba visível**. Componente `AutoRefresh` (`src/components/auto-refresh.tsx`) chama `router.refresh()` e é montado na `page.tsx`, atualizando "Minhas OS" e "Painel do Setor" (Server Components) sem recarregar a página. A tela de Relatórios usa o mesmo hook para re-executar `carregarRelatorio` (dados buscados no cliente). Não é tempo real (WebSocket), mas remove a necessidade de F5.
...
- [x] **Anexos na criação:** O modal de Nova OS ganhou um campo de **prints/imagens e documentos** (até 8) — imagens com preview/thumbnail (redimensionadas via `src/lib/image-utils.ts`) e **documentos** (PDF, Word, Excel, CSV, TXT, PowerPoint) exibidos como card com ícone/tipo/tamanho. Tipos permitidos e limites em `src/lib/anexo-tipos.ts`; processamento no cliente em `src/lib/anexo-utils.ts` (imagem comprime, documento lê direto, até 5MB). Tudo salvo como `AnexoOS` (base64).
- [x] **Thread de comentários + galeria na tela de detalhes:** O modal de detalhes da OS (`os-list-container.tsx`) carrega comentários e anexos sob demanda (`obterDetalhesOS`), permite **adicionar comentários e novos anexos (prints ou documentos)** e removê-los. Imagens abrem em lightbox interno; documentos abrem/baixam via blob URL (`abrirAnexo`). Server Actions: `adicionarComentario`, `adicionarAnexoOS`, `excluirComentario`, `excluirAnexoOS` (`src/app/actions.ts`), com validação de tipo permitido e limites de tamanho/quantidade. Os anexos ficam em tabela separada para não pesar as listagens.

### Fase 17: Migração dos Anexos para o Supabase Storage (Concluída)
- [x] **Motivação:** os anexos deixaram de ser gravados em base64 no banco Postgres (limite ~500MB no free tier, inflado ~33% pelo base64) e passaram a ir para o **Supabase Storage** (bucket privado `anexos-os`, ~1GB separado). O banco guarda apenas o caminho do arquivo.
- [x] **Schema (`prisma/schema.prisma`):** `AnexoOS` ganhou `storagePath String?` (caminho no bucket) e `tamanho Int?` (bytes, para exibição); `dados` virou **opcional** (`String?`) para manter os anexos legados em base64 funcionando. Mudança **aditiva e retrocompatível**.
- [x] **Helper de servidor (`src/lib/supabase-storage.ts`):** client admin com a `SERVICE_ROLE_KEY` (import `server-only`), com `uploadAnexoDataUrl` (decodifica o data URL e envia ao bucket em `<ordemId>/<id>.<ext>`), `urlAssinada` (URL assinada temporária de leitura, 1h), `removerAnexoStorage` e `storageConfigurado`.
...
- [x] **Limpar lidas:** rodapé do painel com **"Limpar notificações lidas"** (aparece só quando há lidas) → action `limparNotificacoesLidas` (`deleteMany` das lidas do próprio usuário).
...
### Fase 23: Redefinição de Senha pelo Admin (Concluída)
...
- [x] **Gráficos removidos:** o `ChartsSection` (recharts) chegou a ser ligado no dashboard, mas foi **removido a pedido** (o componente `charts-section.tsx` foi apagado). A dependência `recharts` ficou órfã no `package.json` (pode ser removida quando quiserem).
